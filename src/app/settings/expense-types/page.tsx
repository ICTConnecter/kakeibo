...
import { ExpenseType } from '@/types/firestore/ExpenseType.d';

type ModalMode = 'add' | 'edit' | 'delete' | null;

export default function ExpenseTypesPage() {
    const { expenseTypes, loading, error, refetch, householdId } = useContext(HouseholdContext);
    ...
    const fetchAllExpenseTypes = async () => {
        ...
        try {
            const response = await fetch(`/api/expense-types?householdId=${householdId}&includeDeleted=true`, {
                headers: {
            ...
            if (result.success && result.data) {
                setAllExpenseTypes(result.data);
            }
        } catch (err) {
            console.error('Failed to fetch all expense types:', err);
        }
    };
    ...
    // 表示する経費タイプ
    const displayExpenseTypes = showDeleted ? allExpenseTypes : expenseTypes;

    // モーダルを開く
    const openAddModal = () => {
        setFormData({ name: '', icon: '💼', color: '#F59E0B', isDefault: false });
    ...
    const openDeleteModal = (expenseType: ExpenseType) => {
        setSelectedExpenseType(expenseType);
        setErrorMessage(null);
        setModalMode('delete');
    };
    ...
    const handleAdd = async () => {
        if (!householdId || !formData.name.trim()) {
            setErrorMessage('経費タイプ名を入力してください');
            return;
        }
        ...
        try {
            ...
            if (!response.ok || !result.success) {
                throw new Error(result.error || '経費タイプの追加に失敗しました');
            }
    ...
    const handleEdit = async () => {
        if (!selectedExpenseType || !formData.name.trim()) {
            setErrorMessage('経費タイプ名を入力してください');
            return;
        }
        ...
        try {
            const response = await fetch(`/api/expense-types/${selectedExpenseType.expenseTypeId}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer:${idToken}`,
                    'Content-Type': 'application/json',
            ...
            if (!response.ok || !result.success) {
                throw new Error(result.error || '経費タイプの更新に失敗しました');
            }
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (!response.ok || !result.success) {
                throw new Error(result.error || '経費タイプの削除に失敗しました');
            }
        ...
        <UserAuthComponent>
            <div className="min-h-screen bg-gray-50 pb-20">
                ...
                <header className="bg-white shadow-sm p-4 flex items-center">
                    ...
                    <h1 className="text-2xl font-bold text-gray-800">経費タイプ管理</h1>
                </header>
                ...
                <main className="max-w-7xl mx-auto p-4 space-y-4">
                    ...
                    <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded">
                        <p className="text-sm">
                            経費タイプは、事業用の支出を分類するために使用します。
                            経費として計上しない場合は「なし」を選択できます。
                        </p>
                    </div>
                    ...
                    {!loading && (
                        <div className="bg-white rounded-lg shadow overflow-hidden">
                            <h2 className="px-6 py-4 font-semibold border-b">経費タイプ一覧</h2>
                            <div className="divide-y">
                                {displayExpenseTypes.length === 0 ? (
                                    ...
                                    displayExpenseTypes
                                        ...
                                        .map((expenseType) => {
                                            const isDeleted = expenseType.status === 'deleted';
                                            return (
                                                <div
                                                ...
                                                >
                                                    <div className="flex items-center gap-3">
                                                        <span
                                                            ...
                                                            style={{ backgroundColor: `${expenseType.color}20` }}
                                                        >
                                                            {expenseType.icon}
                                                        </span>
                                                        <div>
                                                    ...
                                                    <div className="flex gap-2">
                                                        {!isDeleted && (
                                                            <>
                                                                <button
                                                                    onClick={() => openEditModal(expenseType)}
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                        <div className="bg-white rounded-lg p-6 max-w-md w-full">
                            <h2 className="text-xl font-bold mb-4">
                                {modalMode === 'add' ? '経費タイプを追加' : '経費タイプを編集'}
                            </h2>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                                    ...
                                    <span className="font-medium">{formData.name || '経費タイプ名'}</span>
                                    {formData.isDefault && (
                                        <span className="px-2 py-0.5 text-xs bg-green-100 text-green-700 rounded">
                                            デフォルト
                                        </span>
                ...
                {/* 削除確認モーダル */}
                {modalMode === 'delete' && selectedExpenseType && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                        <div className="bg-white rounded-lg p-6 max-w-md w-full">
                            <h2 className="text-xl font-bold mb-4">経費タイプを削除</h2>
                            ...
                            <p className="text-gray-700 mb-2">
                                以下の経費タイプを削除してもよろしいですか？
                            </p>

                            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg mb-6">
                                <span
                                    className="text-2xl w-10 h-10 rounded-full flex items-center justify-center"
                                    style={{ backgroundColor: `${selectedExpenseType.color}20` }}
                                >
                                    {selectedExpenseType.icon}
                                </span>
                                <span className="font-medium">{selectedExpenseType.name}</span>
                                {selectedExpenseType.isDefault && (
                                    <span className="px-2 py-0.5 text-xs bg-green-100 text-green-700 rounded">
                                        デフォルト
                                    </span>
                                )}
                            ...
                            <p className="text-sm text-red-600 mb-4">
                                この操作は取り消せません。この経費タイプに紐づく取引データは影響を受ける可能性があります。
                            </p>
